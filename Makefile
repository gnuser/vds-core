...
CONFIG_CLEAN_FILES = libvdsconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/vds-config.h.in \
...
CXXFLAGS = -std=c++11 -pipe -O1 -fwrapv -fno-strict-aliasing -Werror -g  -Wformat -Wformat-security -fPIE -fopenmp -fwrapv -fno-strict-aliasing 
CYGPATH_W = echo
DEFS = -DHAVE_CONFIG_H
DEPDIR = .deps
DLLTOOL = false
...
HARDENED_CXXFLAGS =  -Wformat -Wformat-security  -fPIE
...
LIBTOOL_APP_LDFLAGS = 
LIBVDS_LIBS = -lgmp -lgmpxx -lboost_system -lcrypto -lsodium -fopenmp -lrustzcash -ldl -lz
LIPO = 
LN_S = ln -s
LRELEASE = 
LTLIBOBJS = 
...
OBJCXXFLAGS = -std=c++11 -pipe -O1 -fwrapv -fno-strict-aliasing -Werror -g  -Wformat -Wformat-security  -fPIE
OBJDUMP = objdump
OBJEXT = o
OTOOL = 
OTOOL64 = 
...
abs_builddir = /home/ubuntu/vds/vds-core
abs_srcdir = /home/ubuntu/vds/vds-core
...
#pkgconfig_DATA = libvdsconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/vdsd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/$(BITCOIN_GUI_NAME)$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/vds-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  ...
  vds-gtest.info vds-gtest_filtered.info vds-gtest_coverage.info

dist_bin_SCRIPTS = vcutil/fetch-params.sh
dist_noinst_SCRIPTS = autogen.sh vcutil/build-debian-package.sh vcutil/build.sh
EXTRA_DIST = $(top_srcdir)/share/genbuild.sh qa/pull-tester/rpc-tests.sh qa/pull-tester/run-bitcoin-cli qa/rpc-tests qa/vds $(DIST_DOCS) $(BIN_CHECKS)
...
src/config/vds-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/vds-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/vds-config.h
$(top_srcdir)/src/config/vds-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/vds-config.h src/config/stamp-h1
libvdsconsensus.pc: $(top_builddir)/config.status $(srcdir)/libvdsconsensus.pc.in
...
#vds-gtest.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ vds-gtest_check
#	$(LCOV) -c -d $(abs_builddir)/src -t vds-gtest -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#vds-gtest_filtered.info: vds-gtest.info
#	$(LCOV) -r $< "/usr/include/*" \
#                      "$(abs_builddir)/depends/x86_64-unknown-linux-gnu/include/*.h" \
#                      "$(abs_builddir)/depends/x86_64-unknown-linux-gnu/include/boost/*" \
#                      "$(abs_builddir)/depends/x86_64-unknown-linux-gnu/include/gmock/*" \
...
#vds-gtest_coverage.info: baseline_filtered_combined.info vds-gtest_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a vds-gtest_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_bitcoin_filtered.info vds-gtest_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitcoin_filtered.info -a vds-gtest_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitcoin.coverage/.dirstamp:  test_bitcoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@

#vds-gtest.coverage/.dirstamp:  vds-gtest_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@

#cov-vds: vds-gtest.coverage/.dirstamp

#total.coverage/.dirstamp: total_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@

#cov: test_bitcoin.coverage/.dirstamp cov-vds total.coverage/.dirstamp

install-exec-hook:
	mv $(DESTDIR)$(bindir)/fetch-params.sh $(DESTDIR)$(bindir)/vds-fetch-params

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_bitcoin.coverage/ vds-gtest.coverage/ total.coverage/ $(OSX_APP)
